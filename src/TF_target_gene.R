...
#
MCF7 <- read.table("MCF7_ASB", header = T)
...
gene <- read.table("gene_ID1", header = F)
aa <- c()
for(i in 1:nrow(gene)){
  getBM(c("hgnc_symbol", "chromosome_name", "start_position", "end_position", "ensembl_gene_id"), "ensembl_gene_id", gene$V1[i], ensembl) -> a
  rbind(aa, a) -> aa
}
write.table(aa, "gene_ID2_region", quote=FALSE, row.names=FALSE, col.names= F, sep = '\t')
#
gene <- read.table("gene_ID2", header = F)
aa<-c()
for(i in 1:nrow(gene)){
...
write.table(aa, "transcript_ID2_region", quote = FALSE, row.names = FALSE, col.names = F, sep = '\t')
####### exon_region
gene1 <- read.table("gene_ID1", header = F)
gene2 <- read.table("gene_ID2", header = F)
...
write.table(aa, "gene_ID_exon_region", quote=FALSE, row.names=FALSE, col.names= F, sep = '\t')
####
gene_with_snp <- read.table("ASE.txt", header = F)
...
write.table(gene_with_snp, "gene_with_snp", quote=FALSE, row.names=FALSE, col.names= F, sep = '\t')
##### promotor_region
gene <- read.table("ASB_in_promotor", header = F)
aa<-c()
for(i in 1:nrow(gene)){
...
##### binomial test
gene <- read.table("target_gene", header = F)
aa<-c()
for(i in 1:nrow(gene)){
...
fdr$lfdr -> fdr
write.table(fdr, "fdr.txt", quote = FALSE, row.names = FALSE, col.names = FALSE)

write.table(aa, "target_gene_binomialtest", quote = FALSE, row.names = FALSE, col.names = F, sep = '\t')
